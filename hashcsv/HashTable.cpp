...
unsigned int HashTable::hash(int key) {
    ...
    for(int i = 0;i<DEFAULT_SIZE;i++)
        if(table[(key+i) % DEFAULT_SIZE].amount==0.0)
...
void HashTable::Insert(Bid bid) {
    ...
    unsigned int hashval = hash(stoi(bid.bidId));
    if(hashval!=DEFAULT_SIZE)
        table[hashval] = bid;
}
    ...
    */
    ...
    try {
        ...
        for (unsigned int i = 0; i < file.rowCount(); i++) {
            ...
            //std::cout << bid.title << bid.fund << bid.amount << std::endl;
