...
int partition(vector<Bid>& bids, int begin, int end) {
	...
	for (int j = begin; j <= end - 1; j++) {
		if (bids.at(j).title.compare(bids.at(pivot).title) < 0) {
			i++;
			swap (bids.at(i), bids.at(j));
		}
	}
	swap (bids.at(i+1), bids.at(end));
	return i+1;
}
/*
	int low = begin;
	...
	while (!done) {
		...
		if (low >= high) {
			...
			// swap in low and high bids using built-in vector
...
//partition bids into low and high part
