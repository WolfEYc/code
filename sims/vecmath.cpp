...
Vector2f pivotfromangle(float mirrorangle, float shotangle, Vector2f rotationPoint, float force){
    ...
    float X = force*cos(degs2rads(pivotangle-90.f))+rotationPoint.x;
    float Y = force*sin(degs2rads(pivotangle-90.f))+rotationPoint.y;
...
Vector2f newPivot(Vector2f a1, Vector2f pivot, surface wall){
    ...
    Vector2f walla = wall.first, wallb = wall.second;

    Vector2f ipoint = calcIntersectVector(a1, pivot, walla, wallb);

    float force = dist(pivot - a1);

    float shotangle = rotationAngle(a1, ipoint);
    
    float mirrorangle = rotationAngle(walla, wallb);    

    return pivotfromangle(mirrorangle, shotangle, ipoint, force);
}
...
surface extendLine(const Vector2f p1, const Vector2f p2, const float scale){
    ...
    float sx = scale*cos(theta);
    float sy = scale*sin(theta);

    Vector2f sP1 = signsVector(p2, p1);
    Vector2f sP2 = signsVector(p1, p2);

    sP1 = p1 + Vector2f(sP1.x*sx, sP1.y*sy);
    sP2 = p2 + Vector2f(sP2.x*sx, sP2.y*sy);
...
int main(){
    Vector2f a1(5.f,4.f);
    Vector2f a2(12.f,8.f);
    Vector2f b1(0.f,5.f);
    Vector2f b2(15.f,5.f);

    Vector2f bean = calcIntersectVector(a1, a2, b1 ,b2);
